...
model of CAKE, implemented in the \texttt{traffic-control} module. The model
...
Algorithm~\ref{alg:dequeue} states the shaping-mode dequeue procedure.
...
bottleneck rather than just as an AQM: it installs CAKE as a $10$~Mbit/s shaper
on a $100$~Mbit/s link with three long-lived Cubic flows ($22$~ms base RTT).
The deficit-mode shaper holds the aggregate at the configured $10$~Mbit/s, the
three flows share the link to within a few percent, and the FlowMonitor mean
per-flow delay stays at roughly the base RTT---only a few milliseconds of
queueing on top of the propagation delay. This is the same mechanism the
validation experiment of Section~\ref{sec:validation} uses, just with the
shaper, rather than the link, as the bottleneck.
...
validation harness are released for inclusion in ns-3.
